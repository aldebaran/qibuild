...
function(create_cmakemodule_lib target)
  qi_debug("create_cmakemodule_lib: (${target})")

  _qi_create_cmake_module(_target VARS INCLUDE_DIRS DEFINITIONS LIBRARIES DEPENDS)
  ...
  configure_file("${QI_TEMPLATE_DIR}/findmodule.lib.cmake.in"
                 "${QI_SDK_DIR}/${QI_SDK_CMAKE_MODULES}/${_NAME}Config.cmake" @ONLY)


  #TODO: use relative path (otherwize layout dont work)
  set(_MODULE_INCLUDE_DIR "\${_ROOT_DIR}/../../../${QI_SDK_INCLUDE}/")
  if (${_NAME}_HEADER_SUBFOLDER)
    foreach(_h ${${_NAME}_HEADER_SUBFOLDER})
      set(_MODULE_INCLUDE_DIR "\${_ROOT_DIR}/../../../${QI_SDK_INCLUDE}/${_h}/" ${_MODULE_INCLUDE_DIR})
    endforeach(_h ${${_NAME}_HEADER_SUBFOLDER})
  endif (${_NAME}_HEADER_SUBFOLDER)

  configure_file("${QI_TEMPLATE_DIR}/findmodule.lib.cmake.sdk.in"
  ...
  if (${_target}_NO_INSTALL)
    ...
    install_cmake("modules/" "${CMAKE_BINARY_DIR}/${QI_SDK_CMAKE_MODULES}/sdk/${_NAME}Config.cmake")
  endif()
endfunction()
...
function(create_root)
  qi_debug("SDK: create_root: (${_name}, ${_prefix}, ${_header})")
