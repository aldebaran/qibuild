...
def test_only_gerrit_no_review(git_worktree, git_server):
    git_server.create_repo("foo.git", review=True, only_review=True)
    res = git_worktree.configure_manifest(git_server.manifest_url, review=False)
    assert not res
