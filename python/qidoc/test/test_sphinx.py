...
def test_spellcheck(doc_worktree, record_messages):
    ...
    doc_builder.configure()
    # pylint:disable-msg=E1101
    with pytest.raises(qidoc.sphinx_project.SpellingErrors) as e:
        doc_builder.build()
    as_string = str(e.value)
    assert  "Found 1 spelling error" in as_string
    assert "missstake" in as_string
