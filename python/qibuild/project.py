...
def is_buildable(worktree_project):
    ...
    return (os.path.exists(worktree_project.qiproject_xml) and
...
def update_project(project, toc):
    ...
    if sdk_dir:
        ...
        project._custom_sdk_dir = os.path.normpath(
            os.path.join(project._custom_sdk_dir, bname))
...
def check_worktree(toc, project_name, project_path):
    ...
    answer = qisys.interact.ask_yes_no(
        question % (project_name, toc.worktree.root), default=True)
    if answer:
