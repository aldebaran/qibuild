...
def test_names_no_worktree(cd_to_tmpdir, args):
    ...
    assert str(e.value) == "Cannot use project names when running outside a worktree"
...
def test_no_worktree_no_args(cd_to_tmpdir, args):
    ...
    with pytest.raises(Exception) as e:
        qilinguist.parsers.get_linguist_projects(args)
    assert str(e.value) == "You should specify at least a pml path when running outside a worktree"
